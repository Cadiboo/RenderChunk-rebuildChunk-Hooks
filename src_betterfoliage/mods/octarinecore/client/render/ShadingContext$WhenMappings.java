package mods.octarinecore.client.render;

import kotlin.Metadata;
import net.minecraft.util.EnumFacing;

// $FF: synthetic class
@Metadata(
   mv = {1, 1, 9},
   bv = {1, 0, 2},
   k = 3
)
public final class ShadingContext$WhenMappings {
   // $FF: synthetic field
   public static final int[] $EnumSwitchMapping$0 = new int[EnumFacing.values().length];

   static {
      $EnumSwitchMapping$0[EnumFacing.UP.ordinal()] = 1;
      $EnumSwitchMapping$0[EnumFacing.DOWN.ordinal()] = 2;
      $EnumSwitchMapping$0[EnumFacing.NORTH.ordinal()] = 3;
      $EnumSwitchMapping$0[EnumFacing.SOUTH.ordinal()] = 4;
      $EnumSwitchMapping$0[EnumFacing.EAST.ordinal()] = 5;
      $EnumSwitchMapping$0[EnumFacing.WEST.ordinal()] = 6;
   }
}
