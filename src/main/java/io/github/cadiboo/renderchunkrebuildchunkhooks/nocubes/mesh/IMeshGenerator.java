...
public interface IMeshGenerator {
	...
	@Nonnull
	FaceList generateBlock(@Nonnull final BlockPos pos, @Nonnull final IBlockReader blockAccess);
					...
					for (int zOffset = 0; zOffset < 2; ++zOffset) {
						for (int yOffset = 0; yOffset < 2; ++yOffset) {
							for (int xOffset = 0; xOffset < 2; ++xOffset) {
								...
								density += CacheUtil.getIndividualBlockDensity(NoCubesTest.isSmoothable(state), state, blockAccess, pooledMutableBlockPos);
							}
						}
					}
