package io.github.cadiboo.renderchunkrebuildchunkhooks.nocubes.mesh.generator;

import io.github.cadiboo.renderchunkrebuildchunkhooks.nocubes.NoCubesTest;
import io.github.cadiboo.renderchunkrebuildchunkhooks.nocubes.mesh.IMeshGenerator;
...
public class OldNoCubes implements IMeshGenerator {
	...
	@Nonnull
	public static HashMap<Vec3b, FaceList> generateChunk(@Nonnull final BlockPos chunkPos, @Nonnull final IBlockReader blockAccess, @Nonnull final PooledMutableBlockPos pooledMutableBlockPos) {
		...
		for (final MutableBlockPos pos : BlockPos.getAllInBoxMutable(chunkPos, chunkPos.add(15, 15, 15))) {
			...
			final FaceList faces = generateBlock(pos, blockAccess, pooledMutableBlockPos);
	...
	@Nonnull
	public static FaceList generateBlock(@Nonnull final BlockPos pos, @Nonnull final IBlockReader blockAccess, @Nonnull final PooledMutableBlockPos pooledMutableBlockPos) {
		...
		final Vec3[] points = getPoints(posX, posY, posZ, relativePosX, relativePosY, relativePosZ, state, blockAccess, pooledMutableBlockPos);
		final FaceList faces = FaceList.retain();

		if (points != null) {
			for (final EnumFacing facing : EnumFacing.values()) {
				if (NoCubesTest.isSmoothable(blockAccess.getBlockState(pooledMutableBlockPos.setPos(pos).offset(facing)))) {
					continue;
				}
	...
	@Nullable
	public static Vec3[] getPoints(final int posX, final int posY, final int posZ, int relativePosX, int relativePosY, int relativePosZ, @Nonnull final IBlockState state, @Nonnull final IBlockReader blockAccess, @Nonnull final PooledMutableBlockPos pooledMutableBlockPos) {

		if (!NoCubesTest.isSmoothable(state)) {
			return null;
		}
...
//				}
	...
	public static boolean doesPointIntersectWithManufactured(@Nonnull final IBlockReader world, @Nonnull final Vec3 point, @Nonnull final PooledMutableBlockPos pooledMutableBlockPos) {
		for (int i = 0; i < 4; i++) {
			...
			if ((!isBlockAirOrPlant(state0)) && (!NoCubesTest.isSmoothable(state0))) {
				return true;
			}
			IBlockState state1 = world.getBlockState(pooledMutableBlockPos.setPos(x1, (int) point.y - 1, z1));
			if ((!isBlockAirOrPlant(state1)) && (!NoCubesTest.isSmoothable(state1))) {
				return true;
			}
		}
		return false;
